...
void moc(const float *joint_membership, const unsigned int *index, const unsigned int *count, struct fuzzy *fuzzy_struct) {
    ...
    int* numerator_count = (int*)malloc(fuzzy_struct->output_num * sizeof(int));
    ...
    for (unsigned int l = 0; l < fuzzy_struct->output_num; ++l) {
        ...
        // printf("numerator_count%d: %d, denominator_count:%f\n", l, numerator_count[l], denominator_count);
        // printf("fuzzy_struct->output[%d]: %f\n", l, fuzzy_struct->output[l]);
#ifdef fuzzy_pid_debug_print
        printf("numerator_count:%f,denominator_count:%f,index:%f\n", numerator_count[l], denominator_count, index[l]);
#endif
    }
    free(numerator_count);// 释放分配的内存
}
...
void fuzzy_control(float e, float de, struct fuzzy *fuzzy_struct) {
    ...
    // printf("Required memory: %d bytes\n", (count[0] * count[1]) * sizeof(float));

#ifdef fuzzy_pid_debug_print
    printf("membership:\n");
    for (unsigned int k = 0; k < j; ++k) {
        printf("membership%d:%f\n", k, membership[k]);
    }
    ...
    size_t requested_size = (count[0] * count[1]) * sizeof(float);
    // printf("Attempting to allocate %zu bytes\n", requested_size);
    float* joint_membership = (float*)malloc(requested_size);
    ...
    for (int i = 0; i < count[0]; ++i) {
        for (int j = 0; j < count[1]; ++j) {
            ...
            // printf("Address: %p,joint_membership%d:%.10f\n", &joint_membership[i * count[1] + j],  i * count[1] + j, joint_membership[i * count[1] + j]);
        }
    }

    // if (joint_membership == NULL) {
    //     printf("joint_membership is NULL\n");
    // }
...
struct PID *fuzzy_pid_init(float *params, float delta_k, unsigned int mf_type, unsigned int fo_type,
    ...
    return raw_fuzzy_pid_init(params[0], params[1], params[2], params[3], params[4], params[5], max_error,
                              ...
                              rule_base, min_fuzzy_pwm_output, middle_fuzzy_pwm_output, max_fuzzy_pwm_output);
}
...
int limit(int value, int max_limit, int min_limit) {
    ...
    if (value < min_limit)
        return min_limit;
        // printf("value:%d,max_limit:%d,min_limit:%d\n", value, max_limit, min_limit);
    return value;
}
...
float fuzzy_pid_control(float real, float idea, struct PID *pid) {
    ...
    // printf("output0000:%f\n\r", pid->fuzzy_struct->output[0]);
    // printf("output1111:%f\n\r", pid->fuzzy_struct->output[1]);
    // printf("output2222:%f\n\r", pid->fuzzy_struct->output[2]);
    // printf("delta_kp : %f\n\rdelta_ki : %f\n\rdelta_kd : %f\n\r", pid->delta_kp, pid->delta_ki, pid->delta_kd);
    ...
    // printf("output:%f\n\r", pid->output);
    // printf("idea:%f,out:%f,real:%f,err0:%f,err1:%f\n", idea, pid->output, real, pid->current_error, idea - real);
    return pid->output;
}
...
float pid_control(float real, float idea, struct PID *pid) {
    ...
    if (pid->linear_adaptive_kp > 1e-4f)
        linear_adaptive_kp =
...
int fuzzy_pid_motor_pwd_output(float real, float idea, bool direct, struct PID *pid) {
    int pid_output = fuzzy_pid_control(real, idea, pid);
    int direct_result = direct_control(pid->output_middle_value, pid_output, direct);
    int limited_result = limit(direct_result, pid->output_max_value, pid->output_min_value);
    
    printf("PID_Out:%d, Direct:%d, Limited:%d\n", pid_output, direct_result, limited_result);
